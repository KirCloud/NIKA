...
struct Player {
    ...
    void setCustomGlow(int health, bool isVisible, bool isSameTeam)
    {
        ...
        std::array<unsigned char, 4> highlightFunctionBits = {
            1,   // InsideFunction
            225, // OutlineFunction: HIGHLIGHT_OUTLINE_OBJECTIVE
            64,  // OutlineRadius: size * 255 / 8
            225   // (EntityVisible << 6) | State & 0x3F | (AfterPostProcess << 7)
        };
        std::array<float, 3> glowColorRGB = { 29, 178, 218 };
        if (isSameTeam) {
            settingIndex = 20;
        } else if (!isVisible) {
            settingIndex = 65;
            glowColorRGB = { 29, 178, 218 }; // knocked enemies // gray
        } else if (health >= 205) {
            settingIndex = 66;
            glowColorRGB = { 29, 178, 218 }; // red shield
        } else if (health >= 190) {
            settingIndex = 67;
            glowColorRGB = { 29, 178, 218 }; // purple shield
        } else if (health >= 170) {
            settingIndex = 68;
            glowColorRGB = { 29, 178, 218 }; // blue shield
        } else if (health >= 95) {
            settingIndex = 69;
            glowColorRGB = { 29, 178, 218 }; // gray shield // cyan 
        } else {
            settingIndex = 70;
            glowColorRGB = { 29, 178, 218 }; // low health enemies // greeen
        }
        ...
        //item Glow
        for (int highlightId = 0; highlightId < 0; highlightId++) {
        const GlowMode newGlowMode = { 0,0,0,0 };
